...
def _run():
    ...
    if result.succeeded:
        ...
        run("nohup {0}/{1} -conf={0}/conf/{2}.yaml -log={3} &".format(put_remote_path, cf.app_name,env.runmode,log_remote_path) ,pty=False, warn_only=True, stdout=sys.stdout, stderr=sys.stdout)
