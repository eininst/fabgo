...
def _n(put_remote_path):
    ...
    if int(run('[ -e "{0}/conf/nginx/nginx.conf" ] && echo 1 || echo 0'.format(put_remote_path))) == 1:
    ...
    run('cp -rf {0}/conf/nginx/{1}.conf {2}/conf/app/{3}-{4}-{1}.conf'.format(put_remote_path, env.runmode,
                                                                              cf.nginx_path,
